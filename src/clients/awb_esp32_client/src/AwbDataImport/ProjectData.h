...
// Created on 19.06.2026 15:32:26
   ...
   const String Pca9685PwmServoName_PWMFakeServo ="PWM Fake Servo";
    ...
    ProjectData(TCallBackErrorOccured errorOccured)
    {
		...
		servos->addServo(Servo("ScsServo-2", new ServoConfig(ServoConfig::ServoTypes::SCS_SERVO, "Servo right", 2, 0, 236, 822, 55, 400, 537, 0, 0, false, false, servo_001_relaxRanges)));
        		...
        		std::vector<RelaxRange> *servo_000_relaxRanges = new std::vector<RelaxRange>();
		servos->addServo(Servo("StsServo-1", new ServoConfig(ServoConfig::ServoTypes::STS_SERVO, "Fake Servo", 3, 0, 0, 1000, 55, 400, 500, 100, 2000, false, false, servo_000_relaxRanges)));


        		std::vector<RelaxRange> *servo_000_relaxRanges = nullptr;
		servos->addServo(Servo("Pca9685PwmServo-1", new ServoConfig(ServoConfig::ServoTypes::PWM_SERVO, "PWM Fake Servo", 1, 64, 1000, 100, -1, -1, 550, 0, 0, false, false, servo_000_relaxRanges)));
    ...
    void addTimelines() 
    {
		...
		servoPoints1->push_back(ServoPoint(ServoPoint("ScsServo-1", 0, 532)));
		...
		servoPoints1->push_back(ServoPoint(ServoPoint("ScsServo-1", 2000, 230)));
		...
		servoPoints1->push_back(ServoPoint(ServoPoint("ScsServo-1", 4000, 821)));
		servoPoints1->push_back(ServoPoint(ServoPoint("ScsServo-1", 6000, 532)));
