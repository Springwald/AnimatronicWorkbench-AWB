...
if (clients.Any())
{
    ...
    while (Console.KeyAvailable == false)
        foreach (var senderReceiver in senderReceivers)
        {
            ...
            if (result.Ok == false)
            {
                Console.WriteLine($"Error sending demo packet {result.OriginalPacketId} to client {senderReceiver.ClientId}: {result.Message}");
            }
            else
            {
                Console.WriteLine($"Sent demo packet {result.OriginalPacketId} to client {senderReceiver.ClientId}: {result.Message}");
            }
